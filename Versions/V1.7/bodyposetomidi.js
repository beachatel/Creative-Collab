...
function draw() {
  ...
  if (millis() - timer >= shootstarInterval) {
    ...
    shootStarPos[0] = random(100, windowWidth - 100);
    shootStarPos[1] = random(100, windowHeight - 100);
    shootStarPos[2] = random(110, 300);
    timer = millis();
  }
  if (shootStarBool == true) {
    tint(255, 50);
    image(
      shootingStar,
      shootStarPos[0],
  ...
  for (let i = 0; i < poses.length; i++) {
    ...
    if (la && ra && la.confidence > 0.3 && ra.confidence > 0.3) {
      let avgAnkleX = (la.x + ra.x) / 2;

      let chordIndex = floor(map(avgAnkleX, 0, width, 0, chordSets.length));
      chordIndex = constrain(chordIndex, 0, chordSets.length - 1);

      let chordNotesAnkle = chordSets[chordIndex];

      chordNotesAnkle.forEach((n) => {
      ...
      Object.keys(activeNotes).forEach((key) => {
        if (key.startsWith("chord_")) {
          let note = parseInt(key.replace("chord_", ""));
